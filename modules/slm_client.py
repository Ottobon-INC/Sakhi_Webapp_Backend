...
class SLMClient:
    ...
    def __init__(
        ...
        self.model_name = model_name or os.getenv("SLM_MODEL_NAME")
        
        print(f"DEBUG: SLM Model being used: {self.model_name}")
        ...
        if self.endpoint_url:
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    # Extract response text using standard OpenAI completions format
        ...
        if self.endpoint_url:
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if "choices" in result and len(result["choices"]) > 0:
                        intent_text = result["choices"][0].get("message", {}).get("content", "")
                    else:
